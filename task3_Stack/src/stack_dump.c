...
/**///	Stack's parameters.
	...
	sprintf(str, "|               %scanary  = %lf%s", blue_col, *(stk->data+0), space_h);	sprintf(str, "%.*s", 51, str);	strcat(str_res, str);	len += getStrLen(str);
	sprintf(str, "%s%s%s", red_col, response, reset_col);								sprintf(str, "%.*s", 20, str);	strcat(str_res, str);	len += getStrLen(str);
	printf("%s%.*s|\n", str_res, SPACE_LEN - (len - blue_l - red_l - reset_l) - 2, space);
	...
	for (int i = 0; i < stk->capacity; i++)
	{
		...
		sprintf(str, ": %10.3lf%s", value, space_h);					sprintf(str, "%.*s", 20, str);	strcat(str_res, str);	len += getStrLen(str);
	...
	len = 0;	memset(str_res, 0, 100);
	response = (state & ST_BAD_CANARYDATA) ? resp_bad : resp_none;
	sprintf(str, "|               %scanary  = %lf%s", blue_col, *(stk->data+stk->capacity+1), space_h);	sprintf(str, "%.*s", 51, str);	strcat(str_res, str);	len += getStrLen(str);
	sprintf(str, "%s%s%s", red_col, response, reset_col);								sprintf(str, "%.*s", 20, str);	strcat(str_res, str);	len += getStrLen(str);
	printf("%s%.*s|\n", str_res, SPACE_LEN - (len - blue_l - red_l - reset_l) - 2, space);
